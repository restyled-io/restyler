...
setupClone :: (HasCallStack, MonadApp m) => PullRequest -> FilePath -> m ()
...
loadRestyledPullRequest
    :: (HasCallStack, MonadApp m) => PullRequest -> m (Maybe SimplePullRequest)
...
-- | Load @.restyled.yaml@
loadConfig :: MonadApp m => m Config
loadConfig = do
