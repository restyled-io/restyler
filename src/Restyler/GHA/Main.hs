module Restyler.GHA.Main () where
...
import Restyler.Commands.RestylePath qualified as RestylePath
...
main = do
  ...
  case cmd of
    -- CloneAndRestylePR options -> CloneAndRestylePR.run options
    -- RestylePR options -> RestylePR.run options
    RestylePaths options -> RestylePath.run options
